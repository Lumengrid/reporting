...
/**
 * @category Manager
 * @summary Get report manager extraction in CSV format
 * @method GET
 ...
 * @url /analytics/v1/manager/report/{report_type_code}/{query_execution_id}/csv
 */
export const reportManagerCsv = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const stream = await info.getS3DownloadStream(session.getS3(), 'csv', session.platform.isDatalakeV3ToggleActive());
        ...
        logger.errorWithStack(`Error while getting csv of the report manager (idReport: ${reportTypeCode}, exportId: ${queryExecutionId}).`, e);
