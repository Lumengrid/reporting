...
/**
 * @category Dashboard
 * @summary Download Branches as CSV
 * @method GET
 * @notes This endpoint download Branches dashboard as csv. Only available for SuperAdmin. Available for datalake v3. Is not feasible to test this API with api-browser. Use postman or similar tools.
 ...
 * @url /analytics/v1/dashboard/branches/{query_execution_id}/exportCsv
 *
 ...
 */
export const getDashboardBranchesExportCsv = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const url = await snowflakeDriver.getSignedUrlExportCsvFromQueryID(queryExecutionId, DashboardTypes.BRANCHES, 'export_branches');
        ...
        logger.errorWithStack(`Error while performing export csv of dashboard branches.`, err);
