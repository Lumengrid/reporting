...
/**
 * @category Dashboard
 * @summary Download Users as CSV
 * @method GET
 * @notes This endpoint download Users dashboard as csv. Only available for SuperAdmin. Available for datalake v3. Is not feasible to test this API with api-browser. Use postman or similar tools.
 ...
 */
export const getDashboardUsersExportCsv = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const url = await snowflakeDriver.getSignedUrlExportCsvFromQueryID(queryExecutionId, DashboardTypes.BRANCHES_USERS, 'export_enrollment');
        ...
        logger.errorWithStack(`Error while performing export csv of dashboard users.`, err);
