...
import { CourseChartsResponse, ErrorResponse } from '../../models/custom-report';

/**
 * @category Dashboard
 * @summary Get course enrollment statistics to be used in charts
 * @method GET
 * @notes This endpoint retrieves the statistics of courses enrollments to be used in charts based on several parameter. Available for datalake v3.
 * @toggle release/weekly/datalake-v3
 * @toggle toggle/admin/report/courses-dashboard-on-athena
 * @url /analytics/v1/dashboard/courses/charts
 *
 ...
 *      @item partecipation [array, required] list of the Top courses by number of enrollments
 *          @item idcourse [integer, optional] Course Id
 *          @item code [string, optional] Course Code
 *          @item name [string, optional] Course Name
 ...
 *      @end
 *      @item completion [array, required] list of Courses completion grouped by date
 *          @item year [integer, optional] Year
 *          @item month [integer, optional] Month
 *          @item date [integer, optional] Date
 *          @item enrolled [integer, optional] The number of course enrollments
 *          @item completed [integer, optional] The number of course enrollments with the status "completed"
 *          @item in_progress [integer, optional] The number of course enrollments with the status "in_progress"
 *          @item not_started [integer, optional] The number of course enrollments that aren't started yet
...
export const getDashboardCoursesCharts = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        response.data = {
            completion: await dashboard.getCoursesCompletion(req),
            partecipation: await dashboard.getTopCoursesByEnrollments(req),
            ...dataCourse
        };
        res.status(200);
        res.json(response);
        ...
        logger.errorWithStack(`Error while performing get dashboard course charts call.`, err);
