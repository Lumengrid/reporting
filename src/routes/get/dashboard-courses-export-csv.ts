...
/**
 * @category Dashboard
 * @summary Download course dashboard as csv
 * @method GET
 * @notes This endpoint download course dashboard as csv. Available for datalake v3. Is not feasible to test this API with api-browser. Use postman or similar tools.
 ...
 * @url /analytics/v1/dashboard/courses/{query_execution_id}/exportCsv
 *
 ...
 */
export const getDashboardCoursesExportCsv = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const url = await snowflakeDriver.getSignedUrlExportCsvFromQueryID(queryExecutionId, DashboardTypes.COURSES, 'export_enrollment');
        ...
        logger.errorWithStack(`Error while performing export csv of dashboard course.`, err);
