...
export class InitializedMessageHandler extends ScheduledReportBaseMessageHandler {
	protected async doHandleMessage(message: IncomingMessage): Promise<void> {
		...
		return applicationService.performQuery(new ExtractionId(this.ExtractionId, this.ReportId));
	}
}
