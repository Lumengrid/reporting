...
export class ReportConversionSkippedMessageHandler extends ScheduledReportBaseMessageHandler {
	protected async doHandleMessage(message: IncomingMessage): Promise<void> {
		...
		return applicationService.compress(new ExtractionId(message.payload.extraction_id, message.payload.report_id));
	}
}
