...
export class CheckExportStatusMessageHandler extends ScheduledReportBaseMessageHandler {
	protected async doHandleMessage(message: IncomingMessage): Promise<void> {
		...
		return applicationService.checkCsvQueryStatus(new ExtractionId(message.payload.extraction_id, message.payload.report_id));
	}
}
